...
bool GuiMLTextCtrl::onAdd()
{
   if(!Parent::onAdd())
      return false;
   if (!mCurrTextSize && mInitialText[0] != 0) 
...
void GuiMLTextCtrl::drawAtomText(bool sel, U32 start, U32 end, Atom *atom, Line *line, Point2I offset)
{
   ...
   if(!sel)
   {
      if(atom->style->shadowOffset.x || atom->style->shadowOffset.y)
      {
         ...
         dglDrawTextN(font, drawPoint + atom->style->shadowOffset, mTextBuffer + start, end - start, mAllowColorChars ? mProfile->mFontColors : NULL);
      }
...
GuiMLTextCtrl::Atom *GuiMLTextCtrl::splitAtomListEmit(Atom *list, U32 width)
{
   ...
   while(list)
   {
      ...
      else
      {
         ...
         if(breakPos == 0 || (breakPos < list->len && mTextBuffer[list->textStart + breakPos - 1] != ' ' && emitted))
            break;
