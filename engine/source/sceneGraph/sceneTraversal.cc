                                ...
                                const U32    objectMask )
{
   ...
   for (i = 0; i < prl.mList.size(); i++)
      prl.mList[i]->setTraversalState( SceneObject::Pending ); //setTraverseColor(SceneObject::White);

   for (i = 0; i < prl.mList.size(); i++)
      if( prl.mList[i]->getTraversalState() == SceneObject::Pending )//if (prl.mList[i]->getTraverseColor() == SceneObject::White)
                                   ...
                                   const U32    stateKey)
{
   ...
   while (pWalk) {
      ...
      if( pOwner->getTraversalState() == SceneObject::Pending )//if (pOwner->getTraverseColor() == SceneObject::White)
