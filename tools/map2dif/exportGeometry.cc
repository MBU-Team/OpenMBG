...
void EditGeometry::fillInLightmapInfo(Surface& rSurface)
{
   ...
   for (U32 i = 0; i < 2; i++) {
      desiredStart[i] = virtualMin[i] / mWorldEntity->mLumelScale;
   ...
   for (U32 i = 0; i < 2; i++) {
      desiredEnd[i] = virtualMax[i] / mWorldEntity->mLumelScale;
   ...
   desiredStart[0] *= mWorldEntity->mLumelScale;
   desiredStart[1] *= mWorldEntity->mLumelScale;
   desiredEnd[0]   *= mWorldEntity->mLumelScale;
   desiredEnd[1]   *= mWorldEntity->mLumelScale;
   ...
   rSurface.tempScale[0] = rSurface.tempScale[1] = 1.0 / mWorldEntity->mLumelScale;

   rSurface.lmapTexGenX[sc] = F32(1.0 / mWorldEntity->mLumelScale);
   rSurface.lmapTexGenX[3]  = F32(-desiredStart[0] / mWorldEntity->mLumelScale);

   rSurface.lmapTexGenY[tc] = F32(1.0 / mWorldEntity->mLumelScale);
   rSurface.lmapTexGenY[3]  = F32(-desiredStart[1] / mWorldEntity->mLumelScale);
...
{
   F64 scaleX = 1.0 / F64(mLightmaps[rSurface.sheetIndex]->getWidth()  * mWorldEntity->mLumelScale);
   F64 scaleY = 1.0 / F64(mLightmaps[rSurface.sheetIndex]->getHeight() * mWorldEntity->mLumelScale);
