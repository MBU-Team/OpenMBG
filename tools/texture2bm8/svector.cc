...
//
extern "C" {
   float asmGetStandardDev(const quantVector* pCenterOfMass,
                           const sortVector*  pVectors,
                           S32              numVectors);
   void asmCalcCenterOfMass(const quantVector* pCenterOfMass,
                            const sortVector*  pVectors,
                            S32        numVectors,
                            float       totalWeight);
   void asmCalcCovarMatrix(const float*     pCenterOfMass,
                           float**          pCaovarMatrix,
                           const U32      numDimension,
                           const sortVector* pVectors,
                           S32             numVectors,
                           float            totalWeight);
   S32 asmFindClosestVectorNonEx(const quantVector* in_pTarget,
                                 const quantVector* in_pSourceVectors,
                                 const U32       in_numSourceVectors,
                                 const U32       in_numDim);
}
                ...
                S32&             io_rNumOutputVectors)
{
   ...
   if (in_numInputVectors <= io_rNumOutputVectors) {
      ...
      for (int i = 0; i < in_numInputVectors; i++) {
         ...
         dMemcpy(out_pOutputVectors[i].pElem, in_pVectors[i].pElem,
