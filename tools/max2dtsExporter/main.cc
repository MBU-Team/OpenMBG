...
S32 _dts_save(const TCHAR *fname, ExpInterface *ei, Interface *gi) 
{
   ...
   if (dStrstr(filename,".dts"))
       pShape->write(&file);
   else if (dStrstr(filename,".dsq"))
       pShape->exportSequences(&file);
   else if (dStrstr(filename,".txt"))
   ...
   if (PDShapeHierarchy & dumpMask && dStrstr(filename,".dts"))
   {
